...
> Last verified by running pytest + ruff (+ aether-eval) on 2026-06-11 01:19:07 MDT.
> Frontend (tsc + web build) last verified 2026-06-09 and unchanged since — Sprint 7 (A+B) touched no frontend.

```yaml
phase: "Sprint 7 - Generality (Permian) IN PROGRESS: Stage B (per-granule k + end-to-end quantification) DONE + green, committed, STOPPED for review. Stage A probe done. Sprint 6 HITRAN-independence migration COMPLETE + green (awaiting human sign-off to formally close)."
status: "In Progress (Sprint 7 Stage B done; Stages C/D gated behind review)"
...
links:
  ...
  key_commits:
    - "HEAD"     # Sprint 7 Stage B - Permian per-granule k + footprint-anchored quantification (gate); resolve via `git log`
...
open_tasks:
  - "Sprint 7 Stage B REVIEW gate (owner: human): the Permian per-granule k + end-to-end quantification is DONE + committed and stopped for review. Read docs/reports/sprint7_stage_b_report.md (esp. the FLAGGED DECISIONS) + docs/science/sprint7_permian.md, then authorize Stage C. Verdict: CROSS-CHECKED earned — Q ours 0.85 t/hr vs NASA's own L2B over the same published-complex-000524 footprint 0.88 t/hr (ratio 0.96x; full-scene Pearson 0.527). Key findings: self-segmentation cannot isolate this WEAK plume (anchored to NASA footprint instead); the +1.46x MF-amplitude systematic does NOT transfer (flips to 0.96x); 18.3 t/hr is context only (NASA's own L2B via this method also ~0.9 t/hr, ~21x below 18.3)."
  - "Sprint 7 Stage C (owner: Claude, GATED on Stage B review): facility-level attribution against the dense committed Permian OGIM subset (12,284 features) with dense-coverage discrimination-honest confidence. STOP for review."
  - "Sprint 7 Stage D (owner: Claude, GATED on Stage C review): UI integration — flip Permian pending->active with visible validation-tier badges; two-event integrity. NOTE: API activation is now gated on assets/<id>/bounds.json (a Stage D artifact), so Permian's committed Stage B q_estimate.json keeps it PENDING until Stage D."
...
validation_status:
  verified_at: "2026-06-11 01:19:07 MDT (fresh run of pytest + ruff + aether-eval)"
  tests: "uv run pytest -> 199 passed, 6 deselected, 2 warnings (exit code 0). +10 vs the prior 189 = Sprint 7 Stage B guards (era5 surface-state unit test, context-only scope-caveat, and Permian no-staleness guards parsing the doc/report headline Q/cross-check/ratio/Pearson vs committed JSON). NOT proof of the science thesis — see 'Validated vs. Unvalidated'."
  ...
  sprint_gate: "Sprint 1 gate PASSED. Sprint 6 gate: operational v2-k migration DONE + green, awaiting human sign-off to close. Sprint 7: Stage A probe DONE; Stage B (per-granule k + footprint-anchored quantification) DONE + green, committed, STOPPED for review (CROSS-CHECKED earned: ours 0.85 vs NASA 0.88 t/hr same footprint). Stages C/D gated."
  eval: "aether-eval run = stub_pipeline, 0/3 recall (baseline, UNCHANGED; real detection not yet registered as the eval pipeline)"
next_milestones:
  - "Human review of the Sprint 7 Stage B gate report (esp. flagged decisions: NASA-footprint anchoring, measured-vs-carried bias, YAML bbox correction, activation gating, scope-caveat parameterization) -> authorize Stage C (dense-coverage facility attribution)."
...
- **Sprint 7 Stage B review** (human): the Permian per-granule k + end-to-end quantification is **done, green, committed, and stopped for review**. Read `docs/reports/sprint7_stage_b_report.md` (especially the **flagged decisions**: NASA-footprint anchoring, measured-vs-carried bias, YAML bbox correction, activation gating, scope-caveat parameterization) + `docs/science/sprint7_permian.md`, then authorize Stage C. Verdict: **CROSS-CHECKED** (ours 0.85 vs NASA-own 0.88 t/hr over the published footprint; +1.46× does not transfer; 18.3 t/hr context only).
- **Sprint 7 Stage C** (Claude, gated on the above): dense-coverage facility attribution against the committed Permian OGIM subset (12,284 features) with discrimination-honest confidence.
- **Sprint 7 Stage D** (Claude, gated on Stage C): UI integration with visible validation-tier badges; two-event integrity. API activation is now gated on `assets/<id>/bounds.json`, so Permian stays PENDING until this stage.
...
> ⚠️ **The 199 passing tests are NOT proof that the core thesis is validated.** They exercise plumbing, schema guards, reproducibility, and the no-fabrication guards. The scientific claims are validated only where explicitly stated below, against real reference data — on a **single event (Goturdepe)**.
>
...
1. Human review of the Sprint 7 Stage B gate report → authorize **Stage C** (dense-coverage facility attribution against the committed Permian OGIM subset).
